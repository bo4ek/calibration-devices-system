...
public abstract class BaseCertificate implements Document {
    ...
    public String getCalibratorCompanyAddress() {
        try {
            ...
            return String.join(", ", (address.getLocality() != null && address.getLocality().substring(0, 4).equals(Constants.KYIV_CITY_NAME.substring(3)) ?
                    address.getLocality().substring(0, 4) : Constants.KYIV_CITY_NAME.substring(3).substring(0, 3) + address.getLocality()), address.getStreet(), address.getBuilding());
        } catch (Exception e) {
